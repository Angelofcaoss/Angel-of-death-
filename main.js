...
/* Creditos a death (https://wa.me/51993966345) */
...
let prekey = []
let directorio = readdirSync("./DeathSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') /*|| file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') */
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./DeathSession/${files}`)
})
} 
...
const directories = ['./DeathSession/', './jadibts/']
...
setInterval(async () => {
  ...
  const bio = `🤖 ᴛɪᴇᴍᴘᴏ ᴀᴄᴛɪᴠᴏ: ${uptime} ┃ 👑 BY ANGEL OF DEATH ┃ 🔗 wa.me/573216796506`;
